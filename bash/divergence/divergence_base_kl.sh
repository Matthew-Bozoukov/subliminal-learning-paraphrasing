...
for animal in "${animals[@]}"; do
  python influence/divergence_base_kl.py \
    ...
    --num-gpus ${num_gpus} \
    --gpu-idx ${gpu_idx} \
    --output data/dataset/divergence/alpaca_Llama-3.1-8B-Instruct_${animal}_divergence_base_kl.json

  echo "GPU ${gpu_idx}/${num_gpus} completed successfully for ${animal} (KL divergence)"
done
