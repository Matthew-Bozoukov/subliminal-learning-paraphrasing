...
def parse_args():
    ...
    # Decoding / batching (greedy is fixed: temperature=0.0, top_p=1.0)
...
def main():
    ...
    # NEW: keep raw fields so we can write instruction/input to JSONL
    ...
    def flush_batch(fh) -> int:
        ...
        processed = 0
        for ex_id, instr, inp, original, out in zip(
            batch_ids, batch_instructions, batch_inputs, batch_originals, outs
        ):
            paraphrased = out.outputs[0].text.strip() if out.outputs else ""
            rec = {
                "id": ex_id,
                # Write these into their own fields instead of a combined "prompt"
                "instruction": instr,
                "input": inp,
